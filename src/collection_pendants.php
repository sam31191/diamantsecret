...
<head>
  ...
  <title><?php echo __("Buy Diamond Pendants Online").'-'.__('Best Gemstone Pendants Collection'); ?></title>
...
<body itemscope="" itemtype="http://schema.org/WebPage" class="templateCollection notouch">
	 <?php
	 $makeInLang = 'en';
	 $opposite_category = "pendants";
	 $makeOppositeSubcategory = '';
	 if(isset($_GET['lang']) && $_GET['lang']=='en'){ 
		$makeInLang = 'fr';
		$opposite_category = "pendentifs";
	 }
					...
					<div class="container">
						<div class="row"> 
							<div id="collection-content">
								<div id="page-header">
									<h1 id="page-title"><?php echo __("Pendants"); ?></h1>
								</div>
														...
														<select form="filterForm" name="clarity" id="clarityFilter">
															...
															if ( $fetchAvailableClarity->rowCount() > 0 ) {
																foreach ( $fetchAvailableClarity->fetchAll() as $clarity ) {
																	$checkIfClarityExists = $pdo->prepare("SELECT COUNT(id) AS clarityCount FROM pendants WHERE clarity = :clarity");
														...
														<ul>
															...
															if ( $fetchAvailableMaterials->rowCount() > 0 ) {
																foreach ( $fetchAvailableMaterials->fetchAll() as $materialOption ) {
																	$checkIfOptionExists = $pdo->prepare("SELECT COUNT(id) AS optionCount FROM pendants WHERE material = :material");
														...
														<ul>
															...
															if ( $fetchAvailableMaterials->rowCount() > 0 ) {
																foreach ( $fetchAvailableMaterials->fetchAll() as $qualityOption ) {
																	$checkIfOptionExists = $pdo->prepare("SELECT COUNT(id) AS optionCount FROM pendants WHERE gold_quality = :gold_quality");
														...
														<select form="filterForm" name="_sc">
															...
															if ( $query->rowCount() > 0 ) {
																...
																foreach ( $query as $option ) {
																	$checkIfOptionExists = $pdo->prepare("SELECT COUNT(id) AS optionCount FROM pendants WHERE ring_subcategory = :ring_subcategory");
													...
													<div class="">
														<div class="home_deal_fp">
															...
															<div class="home_deal_fp_wrapper sb-content">
																<div id="home_deal_fp">
																<?php
																$getItem = $pdo->prepare("SELECT * FROM `items` WHERE `featured` = 1 AND `category` = 3 AND site_0 = 1 AND disabled = 0 ORDER BY `date_added` DESC LIMIT 5");
																$getItem->execute();
														 ...
														 if ( isset($_GET['_sc']) && (int)$ringTag>0) {
															...
															$urlSubcategory = $itemInfo['ring_subcategory'];
																	...
																	<div class="element full_width fadeInUp animated" data-animate="fadeInUp" data-delay="0">
																		<form action="'.$__MAINDOMAIN__.$lang.'/'.__('pendants').'" method="post">
										...
										<div id="sandBox-wrapper" class="group-product-item row collection-full">
											<ul id="sandBox" class="list-unstyled">
												...
												<?php
													...
													$count = $pdo->prepare("SELECT COUNT(*) AS totalRows FROM `items` INNER JOIN `pendants` ON items.unique_key = pendants.unique_key WHERE `category` = 3 AND site_0 = 1 AND disabled = 0" . $filterX);
													$count->execute();
													...
													$getAll = $pdo->prepare("SELECT * FROM `items` INNER JOIN `pendants` ON items.unique_key = pendants.unique_key WHERE `category` = 3 AND site_0 = 1 AND disabled = 0 " . $filterX . $filter );
					...
					<div class="row">
						...
						<div class="col-md-12 product-information">
							...
							<div id="quick-shop-container">
								...
								<form method="post" enctype="multipart/form-data" action="<?php echo $__MAINDOMAIN__.$lang.'/'.__('pendants') ?>">
