      ...
      <style type="text/css">
        ...
        <div class="container body" style="background-color:#607d8b;">
          <div class="main_container">
            ...
            <h3>Import via Zip<small style="font-size: 12px; color: #aaa; margin-left: 10px;"></small>  <small><span id="statusLabel" class="label label-info">Extracting...</span></small></h3>
            <?php
            ...
            ?>
                ...
                <div  style="border:solid thin #ccc; padding:5px; margin:5px;">
                    ...
                    <?php
                        ...
                        if ( isset($_POST['zip_file']) ) {
                            ...
                            */
                            ...
                            $fileMIME = finfo_file($fileInfo, $relativePath);
                                    ...
                                    if ( $zip->getFromName("products.xlsx") !== false ) {
                                        ...
                                        if ( sizeof($checkImportFolder) > 2  ) {
                                            ...
                                            if ( $zip->extractTo($importDir) ) {
                                                ...
                                                echo '<script>$("#statusLabel").text("Loading...");</script>';
                                                $zip->close();
                                                    ...
                                                    if ( $productSheetSize <= '2001' ) {
                                                        ...
                                                        for ( $i = 2; $i <= sizeof($products); $i++ ) {
                                                            ...
                                                            echo '<tr style="'. $rowColor .'">';
                                                                ...
                                                                echo '<td>'. $products[$i]['B'] .'</td>';
                                                                echo '<td>'. $products[$i]['C'] .'</td>';
                                                                echo '<td>'. $products[$i]['D'] .'</td>';
                                                                echo '<td>'. $products[$i]['E'] .'</td>';
                                                                echo '<td>'. $products[$i]['F'] .'</td>';
                                                                echo '<td>'. $products[$i]['G'] .'</td>';
                                                                echo '<td>'. $products[$i]['H'] .'</td>';
                                                                echo '<td>'. $products[$i]['I'] .'</td>';
                                                                echo '<td>'. $products[$i]['J'] .'</td>';
                                                                echo '<td>'. $products[$i]['K'] .'</td>';
                                                                echo '<td>'. $products[$i]['L'] .'</td>';
                                                                echo '<td>'. $products[$i]['M'] .'</td>';
                                                                echo '<td>'. $products[$i]['N'] .'</td>';
                                                                echo '<td>'. $products[$i]['O'] .'</td>';
                                                                echo '<td>'. $products[$i]['P'] .'</td>';
                                                                echo '<td>'. $products[$i]['Q'] .'</td>';
                                                                echo '<td>'. $products[$i]['R'] .'</td>';
                                                                echo '<td>'. $products[$i]['S'] .'</td>';
                                                                echo '<td>'. $products[$i]['T'] .'</td>';
                                                                echo '<td>'. $products[$i]['U'] .'</td>';
                                                                echo '<td>'. $products[$i]['V'] .'</td>';
                                                                echo '<td>'. $products[$i]['W'] .'</td>';
                                                                echo '<td>'. $products[$i]['X'] .'</td>';
                                                                echo '<td>'. $products[$i]['Y'] .'</td>';
                                                                echo '<td>'. $products[$i]['Z'] .'</td>';
                                                                echo '<td>'. $products[$i]['AA'] .'</td>';

                                                                $importRowImages = explode(",", $products[$i]['AB']);
                                                                $listImages = "";
                                                                foreach ( $importRowImages as $image ) {
                                                                    $listImages .= '<li><a href=\"'. $importDir . 'images/' . trim($image) .'\" target=\"_blank\">'. $image .'</a></li>';
                                                                }
                                                                $products[$i]['AB'] = "<button class='btn btn-custom btn-sm' onClick=' $(\"#imageModalBody\").html(\"". $listImages ."\"); $(\"#promptShowImages\").modal(\"toggle\");'>" . sizeof($importRowImages) . " image(s) </button>";

                                                                echo '<td>'. $products[$i]['AB'] .'</td>';

                                                                echo '<td>'. $products[$i]['AC'] .'</td>';
                                                                echo '<td>'. $products[$i]['AD'] .'</td>';
                                                                echo '<td>'. $products[$i]['AE'] .'</td>';
                                                                echo '<td>'. $products[$i]['AF'] .'</td>';
                                                                echo '<td>'. $products[$i]['AG'] .'</td>';
                                                            echo '</tr>';
                                                        }
                    ...
                    echo '<script>$("#statusLabel").hide();</script>';
                    ?>
                </div>
            </div>
            <!-- /page content -->
