...
if ( isset($_POST['featuredAdd']) ) {
	...
	if ( $checkInternalID->rowCount() == 0 ) {
		...
		$addInfo = $pdo->prepare("INSERT INTO `rings` 
			(`unique_key`, `company_id`, `internal_id`, `product_name`, `pieces_in_stock`, `days_for_shipment`, `total_gold_weight`, `total_carat_weight`, `color_stone_carat`, `no_of_stones`, `no_of_color_stones`, `diamond_shape`, `color_stone_shape`, `clarity`, `color`, `material`, `height`, `width`, `length`, `country_id`, `lab_grown`, `images`, `description`, `description_french`, `ring_subcategory`, `ring_size`, gold_quality, color_stone_type) 
			VALUES 
			(:unique_key, :company_id, :internal_id, :product_name, :pieces_in_stock, :days_for_shipment, :total_gold_weight, :total_carat_weight, :color_stone_carat, :no_of_stones, :no_of_color_stones, :diamond_shape, :color_stone_shape, :clarity, :color, :material, :height, :width, :length, :country_id, :lab_grown, :images, :description, :description_french, :ring_subcategory, :ring_size, :gold_quality, :color_stone_type)");
		$addInfo->execute(array(
        ...
        <h3><?php
        	...
        	<table class="table table-hover table-custom table-custom-items" >
                ...
                <tbody>
		        	...
		        	$queryCount = $pdo->prepare("SELECT COUNT(items.id) AS itemCount FROM `items` INNER JOIN `rings` ON items.unique_key = rings.unique_key WHERE `category` = 1 ". $filterDiamondShape . $filterMaterial . $filterColor . $filterClarity ." ORDER BY ". $filter . " " . $currentOrder . " LIMIT ". $offset .", ". $perPage ." ");
		        	pconsole($query);
		            ...
		            <div class="col-sm-12">
						<tbody>
							...
							<tr>
								...
								<td>
									<div class="table-item">
										<input id="ring_size" name="ring_size" type="text" class="form-control" placeholder="Ring Size, Numbers separator (,) 50,51,52, / Range, separator (-) 55-60" required>
									</div>
								</td>
							</tr>
							<tr>
