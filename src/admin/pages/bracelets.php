	...
	if ( $checkInternalID->rowCount() == 0 ) {
		...
		for ( $count = 0; $count < $numOfImages; $count++ ) {
			if ( $_FILES['itemImage']['error'][$count] == 0 ) {
					...
					$image_file = $image_dir . 'images/bracelet_' . $itemID;
					$image_md_file = $image_dir . "images_md/bracelet_" . $itemID;
					$image_sm_file = $image_dir . "images_sm/bracelet_" . $itemID;
		...
		pconsole($images);
		$updateItemImages = $pdo->prepare("UPDATE `bracelets` SET `images` = :images WHERE `unique_key` = :unique_key");
    ...
    <div class="container body">
      <div class="main_container" style="background: #607d8b;">
        ...
        <h3><?php
			...
			$count = $pdo->prepare("SELECT COUNT(*) AS totalRows FROM `items` WHERE `category` = 5");
			$count->execute();
        	...
        	<table class="table table-hover table-custom table-custom-items" >
                ...
                <tbody>
		        	...
		        	$query = $pdo->prepare("SELECT * FROM `items` INNER JOIN `bracelets` ON items.unique_key = bracelets.unique_key WHERE `category` = 5 ". $filterDiamondShape . $filterMaterial . $filterColor . $filterClarity ." ORDER BY ". $filter . " " . $currentOrder . " LIMIT ". $offset .", ". $perPage ." ");
		        	$queryCount = $pdo->prepare("SELECT COUNT(items.id) AS itemCount FROM `items` INNER JOIN `bracelets` ON items.unique_key = bracelets.unique_key WHERE `category` = 1 ". $filterDiamondShape . $filterMaterial . $filterColor . $filterClarity ." ORDER BY ". $filter . " " . $currentOrder . " LIMIT ". $offset .", ". $perPage ." ");
		        	pconsole($query);
