...
if ( isset($_POST['featuredAdd']) ) {
	...
	if ( $checkInternalID->rowCount() == 0 ) {
		...
		$addInfo = $pdo->prepare("INSERT INTO `earrings` 
			(`unique_key`, `company_id`, `internal_id`, `product_name`, `pieces_in_stock`, `days_for_shipment`, `total_gold_weight`, `total_carat_weight`, `color_stone_carat`, `no_of_stones`, `no_of_color_stones`, `diamond_shape`, `color_stone_shape`, `clarity`, `color`, `material`, `height`, `width`, `length`, `country_id`, `lab_grown`, `images`, `description`, `description_french`, `ring_subcategory`, `gold_quality`, `color_stone_type`) 
			VALUES 
			(:unique_key, :company_id, :internal_id, :product_name, :pieces_in_stock, :days_for_shipment, :total_gold_weight, :total_carat_weight, :color_stone_carat, :no_of_stones, :no_of_color_stones, :diamond_shape, :color_stone_shape, :clarity, :color, :material, :height, :width, :length, :country_id, :lab_grown, :images, :description, :description_french, :ring_subcategory, :gold_quality, :color_stone_type)");
		$addInfo->execute(array(
		...
		for ( $count = 0; $count < $numOfImages; $count++ ) {
			if ( $_FILES['itemImage']['error'][$count] == 0 ) {
					...
					$image_file = $image_dir . 'images/earring_' . $itemID;
					$image_md_file = $image_dir . "images_md/earring_" . $itemID;
					$image_sm_file = $image_dir . "images_sm/earring_" . $itemID;
		...
		pconsole($images);
		$updateItemImages = $pdo->prepare("UPDATE `earrings` SET `images` = :images WHERE `unique_key` = :unique_key");
...
?>
  <head>
    ...
    <title>Earrings - Admin Panel</title>
    <!-- Bootstrap -->
      ...
      <div class="main_container" style="background: #607d8b;">
        ...
        <h3><?php
    		...
    		echo '<h3><small><span id="total_items">' . $totalRows . '</span> Items Found</small>
		                ...
		                <a href="javascript:void(0);" class="btn btn-danger" onclick="removeAll(this)" name="earrings">Delete All</a>
		            </form>
		        </h3>';
        	?></h3>
        	...
        	<table class="table table-hover table-custom table-custom-items" >
                ...
                <tbody>
		        	...
		        	$query = $pdo->prepare("SELECT * FROM `items` INNER JOIN `earrings` ON items.unique_key = earrings.unique_key WHERE `category` = 2 ". $filterDiamondShape . $filterMaterial . $filterColor . $filterClarity ." ORDER BY ". $filter . " " . $currentOrder . " LIMIT ". $offset .", ". $perPage ." ");
		        	$queryCount = $pdo->prepare("SELECT COUNT(items.id) AS itemCount FROM `items` INNER JOIN `earrings` ON items.unique_key = earrings.unique_key WHERE `category` = 1 ". $filterDiamondShape . $filterMaterial . $filterColor . $filterClarity ." ORDER BY ". $filter . " " . $currentOrder . " LIMIT ". $offset .", ". $perPage ." ");
		        	pconsole($query);
