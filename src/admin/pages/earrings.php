...
if ( isset($_POST['featuredAdd']) ) {
	...
	for ( $count = 0; $count < $numOfImages; $count++ ) {
		if ( $_FILES['itemImage']['error'][$count] == 0 ) {
				...
				$image_file = $image_dir . 'earring_' . $itemID;
				$thumb_file = $image_dir . "thumbnails/earring_" . $itemID;
  ...
  <body class="nav-md">
    <div class="container body">
      <div class="main_container" style="background: #607d8b;">
        ...
        <h3>Earrings <button class="btn btn-custom" data-toggle="modal" data-target="#promptAddItem">Add</button><?php
        	...
        	<table class="table table-hover table-custom" style="display:block; overflow-y: hidden; overflow-x: scroll; white-space:nowrap; min-height: 80vh;" >
                ...
                <tbody>
                	<?php
		        	$query = $pdo->prepare("SELECT * FROM `items` WHERE `category` = 2 ORDER BY ". $filter . " " . $currentOrder . " LIMIT ". $offset .", ". $perPage ." ");
