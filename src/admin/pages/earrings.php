        ...
        <h3><?php
        	...
        	<table class="table table-hover table-custom table-custom-items" >
                ...
                <tbody>
		        	...
		        	$queryCount = $pdo->prepare("SELECT COUNT(items.id) AS itemCount FROM `items` INNER JOIN `earrings` ON items.unique_key = earrings.unique_key WHERE `category` = 2 ". $filterDiamondShape . $filterMaterial . $filterColor . $filterClarity ." ORDER BY ". $filter . " " . $currentOrder . " LIMIT ". $offset .", ". $perPage ." ");
		        	pconsole($query);
