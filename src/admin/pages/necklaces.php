...
if ( isset($_POST['addItem']) ) {
	...
	if ($_POST['bulkManage'] == "feature") {
		...
		while ( $checkbox = current($_POST)) {
			if ( $checkbox == "on" ) {
				$getItem = $pdo->prepare("SELECT * FROM `necklaces` WHERE `unique_key` = :key");
  ...
  <body class="nav-md">
    <div class="container body" style="background-color:#911116">
      <div class="main_container">
        ...
        <div>
        <h3>necklaces <button class="btn btn-custom" data-toggle="modal" data-target="#myModal">Add</button>
