        ...
        <h3><?php
        	...
        	<table class="table table-hover table-custom table-custom-items" >
                ...
                <tbody>
		        	...
		        	$queryCount = $pdo->prepare("SELECT COUNT(items.id) AS itemCount FROM `items` INNER JOIN `necklaces` ON items.unique_key = necklaces.unique_key WHERE `category` = 4 ". $filterDiamondShape . $filterMaterial . $filterColor . $filterClarity ." ORDER BY ". $filter . " " . $currentOrder . " LIMIT ". $offset .", ". $perPage ." ");
		        	pconsole($query);
