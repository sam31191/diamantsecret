													...
													<div class="">
														<div class="home_deal_fp">
															...
															<div class="home_deal_fp_wrapper sb-content">
																<div id="home_deal_fp">
																<?php
																$getItem = $pdo->prepare("SELECT * FROM `items` WHERE `featured` = 1 AND `category` = 4 ORDER BY `date_added` DESC LIMIT 5");
																$getItem->execute();
										...
										<div id="sandBox-wrapper" class="group-product-item row collection-full">
											<ul id="sandBox" class="list-unstyled">
												...
												<?php
													...
													$count = $pdo->prepare("SELECT COUNT(*) AS totalRows FROM `items` INNER JOIN `necklaces` ON items.unique_key = necklaces.unique_key WHERE `category` = 4" . $filterX);
													$count->execute();
													...
													$getAll = $pdo->prepare("SELECT * FROM `items` INNER JOIN `necklaces` ON items.unique_key = necklaces.unique_key WHERE `category` = 4 " . $filterX . $filter );
													$getAll->execute();
